...
class_declaration_statement:
      optional_attributes class_entry_type identifier_not_reserved type_parameter_list extends_from implements_list '{' class_statement_list '}'
          { $$ = Stmt\Class_[$3, ['type' => $2, 'extends' => $5, 'implements' => $6, 'stmts' => $8, 'attrGroups' => $1], ['typeParams' => $4]];
            $this->checkClass($$, #3); }
    | optional_attributes T_INTERFACE identifier_not_reserved type_parameter_list interface_extends_list '{' class_statement_list '}'
          { $$ = Stmt\Interface_[$3, ['extends' => $5, 'stmts' => $7, 'attrGroups' => $1], ['typeParams' => $4]];
...
type_parameter_list:
      /* empty */                                                   { $$ = array(); }
    | T_TARG_START non_empty_type_parameter_list T_TARG_CLOSE       { $$ = $2; }
;

non_empty_type_parameter_list:
      identifier_not_reserved                                       { init($1); }
    | non_empty_type_parameter_list ',' identifier_not_reserved     { push($1, $3); }
;
...
type_without_static:
    ...
    | name type_argument_list                               { $$ = $this->handleBuiltinTypes($1); $1->setAttribute('typeArgs', $2); }
...
type_argument_list:
      T_TARG_START non_empty_type_argument_list T_TARG_CLOSE { $$ = $2; }
;

non_empty_type_argument_list:
      type_argument                                         { init($1); }
    | non_empty_type_argument_list ',' type_argument        { push($1, $3); }
;

type_argument:
      name                                                  { $$ = $1; }
    | name type_argument_list                               { $$ = $1; $1->setAttribute('typeArgs', $2); }
;
